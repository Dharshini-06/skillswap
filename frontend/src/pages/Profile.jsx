...
const Profile = () => {
    ...
    useEffect(() => {
        async function fetchProfile() {
            ...
            try {
                const res = await fetch(`https://://skillswap-ekvn.onrender.com/api/profile/${userId}`);
                if (res.ok) {
        ...
        async function fetchActivities() {
            try {
                const res = await fetch(`https://://skillswap-ekvn.onrender.com/api/user`, {
                            ...
                            <>
                                <button className="btn btn-primary" style={{ marginLeft: '12px' }} onClick={async () => {
                                    ...
                                    try {
                                        ...
                                        if (res.ok) {
                                            // refresh
                                            const refreshed = await (await fetch(`https://://skillswap-ekvn.onrender.com/api/profile/${userId}`)).json();
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', padding: '1rem' }}>
                                <div className="profile-image-preview">
                                    {(profile.profileType === 'image' && (profileImageFile || profile.profileImage)) ? (
                                        <img src={profileImageFile ? URL.createObjectURL(profileImageFile) : `https://://skillswap-ekvn.onrender.com${profile.profileImage}`} alt="Profile" style={{ width: '120px', height: '120px', borderRadius: '50%', objectFit: 'cover', background: 'var(--bg-secondary)', border: '2px solid var(--accent-color)' }} />
                                    ) : (
                            ...
                            {isEditing ? (
                                ...
                                profile.resume || localStorage.getItem('userResumeUploaded') === 'true' ? (
                                    <div className="resume-status">
                                        <FileText size={20} />
                                        <p>Resume uploaded</p>
                                        <a className="btn btn-outline" style={{ width: '100%', display: 'inline-block', marginTop: '1rem' }} href={`https://://skillswap-ekvn.onrender.com${profile.resume}`} target="_blank" rel="noreferrer">View / Download</a>
                                    </div>
                                ) : (
                                    <button className="empty-add-btn" onClick={() => setIsEditing(true)}>
                                        <FileText size={20} />
